...
class GraficoLinear extends StatefulWidget {
  ...
  GraficoLinear(this.cardIndex, this.tipoGrafico, this.indicador,
      this.tipoEmpresa, this.indicadorFinanca,
      {Key? key})
      : super(key: key);
...
class _GraficoLinearState extends State<GraficoLinear> {
  ...
  void didUpdateWidget(GraficoLinear oldWidget) {
    if (widget.tipoEmpresa != oldWidget.tipoEmpresa ||
        widget.tipoEmpresa == oldWidget.tipoEmpresa) {
      ...
      if (widget.tipoGrafico == 'Finanças' &&
          ...
          widget.tipoGrafico == 'Finanças' &&
              oldWidget.tipoGrafico == 'Finanças' ||
          widget.indicadorFinanca != oldWidget.indicadorFinanca) {
        jsonDataDividendos = '';
        jsonDataPatrimonio = '';
        viewDividendos = [];
        viewPatrimonio = [];
      ...
      chartData_inf_boll = [];*/
        ...
        //loadData();
      } else if (widget.tipoGrafico == 'Price' &&
              oldWidget.tipoGrafico == 'Finanças' ||
          widget.tipoGrafico == 'Price' && oldWidget.tipoGrafico == 'Price' ||
          widget.tipoEmpresa != oldWidget.tipoEmpresa ||
          widget.indicador != oldWidget.indicador) {
        /*widget.price = true;
    ...
    if (widget.tipoGrafico == 'Price') {
      ...
      for (var a = 0; a < view.length; a++) {
        ...
        Iterable<RegExpMatch> matches =
            alphanumeric.allMatches(widget.tipoEmpresa);
      ...
      if (widget.price == true) {
        ...
        jsonDataPred = await rootBundle.loadString(
        ...
        for (int a = view[index_empresa]['Close'].length;
            a < (view[index_empresa]['Close'].length) + 13;
            a++) {
          ////////print(a);
          chartDataPred.insert(
              contagem,
              FlSpot(a.toDouble(),
                  viewPred[index_empresa]['resultado'][a.toString()]));
        ...
        for (int a = 0; a < view[index_empresa]['Close'].length; a++) {
          ...
          chartData.insert(a,
              FlSpot(a.toDouble(), view[index_empresa]['Close'][a.toString()]));
        }
      ...
      switch (widget.indicador) {
        case "Média Móvel 14":
          jsonData1 = await rootBundle.loadString(
          ...
          for (int a = 0; a < view2[index_empresa]['media movel'].length; a++) {
            chartData_media_movel.insert(
                a,
                FlSpot(contador,
                    view2[index_empresa]['media movel'][a.toString()]));
            contador += 1;
          }
          ...
          jsonData1Prev = await rootBundle.loadString(
          ...
          int tamanho = view2[index_empresa]['media movel'].length +
              (view2Prev[index_empresa]['media movel'].length) +
              13; /*remover essa soma quando arrumar os dados de media movel */

          //int contador_tamanho = tamanho;
          ////////print(view2[index_empresa]['media movel'].length+ (view2Prev[index_empresa]['media movel'].length-1));
          //////print(tamanho);
          for (int a = view2[index_empresa]['media movel'].length + 13;
              a < (tamanho);
              a++) {
            ////print(view2Prev[index_empresa]['media movel'][a.toString()]);
            chartDataPrev_media_movel.insert(
                contador_prev_media,
                FlSpot(a.toDouble(),
                    view2Prev[index_empresa]['media movel'][a.toString()]));
        ...
        case "Bollinger Bands":
          ...
          //dados:
          jsonData2 = await rootBundle.loadString(
              'asset/Empresas_data/${widget.cardIndex}_bandas_boll.json');
          view3 = jsonDecode(jsonData2);
          double contador = 13;
          for (int a = 0;
              a < view3[index_empresa]['banda sup boll'].length;
              a++) {
            chartData_sup_boll.insert(
                a,
                FlSpot(contador,
                    view3[index_empresa]['banda sup boll'][a.toString()]));
            contador += 1;
          }
          contador = 13;
          for (int a = 0;
              a < view3[index_empresa]['banda inf boll'].length;
              a++) {
            chartData_inf_boll.insert(
                a,
                FlSpot(contador,
                    view3[index_empresa]['banda inf boll'][a.toString()]));
            contador += 1;
          }
          //Dados prev_inf:
          jsonData2Prev = await rootBundle.loadString(
          ...
          int tamanho = view3[index_empresa]['banda sup boll'].length +
              (view3Prev[index_empresa]['banda sup boll'].length);
          int contador_tamanho = tamanho;
          for (int a = view3[index_empresa]['banda sup boll'].length;
              a < (tamanho);
              a++) {
            chartDataPrev_sup_boll.insert(
                contador_prev_sup_boll,
                FlSpot(double.parse(contador_tamanho.toString()),
                    view3Prev[index_empresa]['banda sup boll'][a.toString()]));
          ...
          tamanho = view3[index_empresa]['banda inf boll'].length +
              (view3Prev[index_empresa]['banda inf boll'].length);
          contador_tamanho = tamanho;
          for (int a = view3[index_empresa]['banda inf boll'].length;
              a < (tamanho);
              a++) {
            chartDataPrev_inf_boll.insert(
                contador_prev_inf_boll,
                FlSpot(double.parse(contador_tamanho.toString()),
                    view3Prev[index_empresa]['banda inf boll'][a.toString()]));
        ...
        case "RSI":
          ...
          jsonData4 = await rootBundle
              .loadString('asset/Empresas_data/${widget.cardIndex}_rsi.json');
          view4 = jsonDecode(jsonData4);
          //////print(view4[index_empresa]['Ind-forc-relat']["1"]);
          double contador = 13;
          for (int a = 0;
              a < view4[index_empresa]['Ind-forc-relat'].length;
              a++) {
            chartData_rsi.insert(
                a,
                FlSpot(contador,
                    view4[index_empresa]['Ind-forc-relat'][a.toString()]));
            contador += 1;
          }

          jsonData4Prev = await rootBundle.loadString(
              'asset/Empresas_data/${widget.cardIndex}_rsi_predicao.json');
          view4Prev = jsonDecode(jsonData4Prev);
          int contador_prev_rsi = 0;

          int tamanho = view4[index_empresa]['Ind-forc-relat'].length +
              (view4Prev[index_empresa]['Ind-forc-relat'].length);
          int contador_tamanho = tamanho;
          ////////print(view2[index_empresa]['media movel'].length+ (view2Prev[index_empresa]['media movel'].length-1));
          //////print(tamanho);
          for (int a = view4[index_empresa]['Ind-forc-relat'].length;
              a < (tamanho);
              a++) {
            chartDataPrev_rsi.insert(
                contador_prev_rsi,
                FlSpot(double.parse(contador_tamanho.toString()),
                    view4Prev[index_empresa]['Ind-forc-relat'][a.toString()]));
            contador_prev_rsi += 1;
            contador_tamanho += 1;
          }
        ...
        case 'MACD':
          //Dados:
          jsonData5 = await rootBundle
              .loadString('asset/Empresas_data/${widget.cardIndex}_macd.json');
          view5 = jsonDecode(jsonData5);

          double contador = 13;
          for (int a = 0; a < view5[index_empresa]['macd'].length; a++) {
            chartData_macd.insert(a,
                FlSpot(contador, view5[index_empresa]['macd'][a.toString()]));
            contador += 1;
          }

          jsonData5Prev = await rootBundle.loadString(
              'asset/Empresas_data/${widget.cardIndex}_macd_predicao.json');
          view5Prev = jsonDecode(jsonData5Prev);
          int contador_prev_macd = 0;

          int tamanho = view5[index_empresa]['macd'].length +
              (view5Prev[index_empresa]['macd'].length);
          int contador_tamanho = tamanho;
          ////////print(view2[index_empresa]['media movel'].length+ (view2Prev[index_empresa]['media movel'].length-1));
          //////print(tamanho);
          for (int a = view5[index_empresa]['macd'].length;
              a < (tamanho);
              a++) {
            chartDataPrev_macd.insert(
                contador_prev_macd,
                FlSpot(double.parse(contador_tamanho.toString()),
                    view5Prev[index_empresa]['macd'][a.toString()]));
                ...
                tooltipBehavior: TooltipBehavior(
                  ...
                  builder: (dynamic data, dynamic point, dynamic series,
                    ...
                    if (seriesIndex == 0) {
                      ...
                      closeValue = view[index_empresa]['Close']
                              [dataX.toString()]
                          .toStringAsFixed(2);
                      ...
                      closeValuePred = viewPred[index_empresa]['resultado']
                              [dataXPred.toString()]
                          .toStringAsFixed(2);

                      dataValuePred =
                          viewPred[index_empresa]['data'][dataXPred.toString()];
                      ////print(viewPred[index_empresa]['resultado'][dataXPred.toString()]);
                    } else if (seriesIndex == 2) {
                      //media movel
                      dataX = chartData[pointIndex].x.toInt();
                      //dataMediaPred = chartDataPrev_media_movel[pointIndex].x.toInt();
                      mediaMovelValue = view2[index_empresa]['media movel']
                              [dataX.toString()]
                          .toStringAsFixed(2);
                    } else if (seriesIndex == 3) {
                      //media movel pred
                      dataMediaPred =
                          chartDataPrev_media_movel[pointIndex].x.toInt();
                      //print(dataMediaPred);
                      dataValuePred = viewPred[index_empresa]['data']
                          [dataMediaPred.toString()];
                      mediaMovelPredValue = view2Prev[index_empresa]
                              ['media movel'][dataMediaPred.toString()]
                          .toStringAsFixed(2);
                ...
                final macdValue = view5[index_empresa]['macd'][dataMACD.toString()].toStringAsFixed(2);*/
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          if (widget.price && indexLine == 0)
                            Text(
                                'Data: ${view[index_empresa]['Data'][dataX.toString()]}'),
                          if (widget.price && indexLine == 0)
                            Text('Close: $closeValue'),
                          if (widget.price && indexLine == 1)
                            Text('Data: $dataValuePred'),
                          if (widget.price && indexLine == 1)
                            Text('Pred. Close: $closeValuePred'),
                          if (widget.price && indexLine == 2)
                            Text(
                                'Data: ${view[index_empresa]['Data'][dataX.toString()]}'),
                          if (widget.mediaVisible && indexLine == 2)
                            Text('Média Móvel: $mediaMovelValue'),
                          if (widget.price && indexLine == 3)
                            Text('Data: $dataValuePred'),
                          if (widget.mediaVisible && indexLine == 3)
                            Text('Pred. Média Móvel: $mediaMovelPredValue'),
